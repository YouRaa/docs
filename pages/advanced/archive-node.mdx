...
1. Установите последнюю версию mina. Если вы не обновились до последней версии daemon, перейдите [назад к документации](/docs/getting-started), чтобы получить последнюю версию. Вы можете запустить код ```mina -help```, чтобы проверить успешность установки.
2. Установите [Postgres](https://www.postgresql.org/download/).
3. Установите пакет архивной ноды. 

    - Ubuntu/Debian:
      ```
      sudo apt-get install mina-archive=1.1.3-48401e9
      ```
...
Ниже приведены базовые инструкции для того, чтобы настроить все необходимое для локальной работы архивной ноды. Они будут немного отличаться, если вы подключаетесь к postgres через облако, если вы спользуете Docker или если вы хотите запускать эти процессы на разных устройствах.
...
1. Запустите локальный сервер postgres. Это просто запустит процесс тестирования, и вы, вероятно, захотите запустить его в фоновом режиме или использовать диспетчер служб вашей ОС (например systemd), чтобы запустить его автоматически. В качестве альтернативы вы можете использовать облачный сервис postgres.
...
Теперь, когда у нас запущена архивная нода, давайте взглянем на таблицы в базе данных.
...
Эта таблице отслеживает награды, полученные за работу snark или за создание блоков.
```
...
...
Теперь, когда мы взглянули на структуру данных, давайте попробуем произвести запрос. 
