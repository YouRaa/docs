...
1. Установите последнюю версию mina. Если вы не обновились до последней версии daemon, перейдите [назад к документации](/docs/getting-started), чтобы получить последнюю версию. Вы можете запкстить код ```mina -help```, чтобы проверить успешность установки.
...
Ниже приведены некоторые базовые инструкции о том, как настроить все необходимое для локальной работы архивной ноды. Они будут немного отличаться, если вы подключаетесь к postgres через облако, если вы спользуете Docker или если вы хотите запускать эти процессы на разных устройствах.
