...
Это клиентский SDK для NodeJS, который позволяет генерировать пары ключей Mina и подписывать транзакции и строки с помощью пар ключей Mina.
Проект содержит типизацию Typescript и ReasonML, но также может использоваться из простого NodeJS.This is a NodeJS client SDK that allows you to generate Mina key pairs and sign transactions and strings using Mina's key pairs.
