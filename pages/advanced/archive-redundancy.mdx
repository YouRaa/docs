...
Эта информация отображается в логах и передается на поток stdout. Мы можем предоставить инструменты фильтрации и получения только этих сообщений для их безопасного и постоянного хранения.

## B. GraphQL JSON Снапшоты

GraphQL сервер daemom Mina размещен на порту `0xc0d` или 3085 (по умолчанию, настраивается с помощью файла конфигурации или флага -rest-server-port) на вашем устройстве (вы можете открыть его для 0.0.0.0 с помощью -insecure-reset-server). Открытие этого порта для общего доступа в Интернет небезопасно.
