...
## B. JSON-Снапшоты GraphQL

GraphQL сервер daemom Mina размещен на порте `0xc0d` или 3085 (по умолчанию, настраивается с помощью файла конфигурации или флага -rest-server-port) на вашем устройстве (вы можете открыть его для 0.0.0.0 с помощью -insecure-reset-server). Открытие этого порта для общего доступа в Интернет небезопасно.
...
В настоящее время в тестовых сетях существует множество приложений, которые используют эту информацию. O(1) labs полагается на этот источник данных в нескольких местах, и, вероятно, в основной сети будут новые пользователи этих данных.

## C. Официальная Архивная Нода PostgreSQL

O(1) labs будет поддерживать восстановление данных из резервных копий (B) или (A).

Преимущество архивной ноды состоит в том, что в сети уже есть пользователи, которые используют запросы вида: «какой блок находится на высоте N в канонической цепи». Смотрите [Https://minaprotocol.com/docs/archive-nodepting](https://minaprotocol.com/docs/archive-node) для получения дополнительной информации.
